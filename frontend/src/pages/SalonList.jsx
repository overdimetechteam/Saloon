  ...
  return (
    ...
    >
      <Link to={`/salons/${salon.id}`} style={{ ...s.card, ...(isFav ? s.cardFav : {}), flex: 1 }}>
        {/* Photo banner */}
        <div style={{ ...s.banner, overflow: 'hidden', position: 'relative', background: `linear-gradient(135deg, ${c1}66 0%, ${c2}44 50%, ${c1}33 100%)` }}>
          <div style={{ ...s.orb, background: `radial-gradient(circle, ${c1}66 0%, transparent 70%)` }} />
          {coverPhoto && (
            <img src={coverPhoto} alt={salon.name} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block', opacity: 0.48, mixBlendMode: 'luminosity' }} />
          )}
          {/* Soft bottom-fade so logo/badge stay readable */}
