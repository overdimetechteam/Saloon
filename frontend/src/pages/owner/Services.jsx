...
/** Split total minutes into { h, m } — rounds m to nearest 5 */
function splitDur(total) {
  const t = Number(total) || 0;
  return { h: Math.floor(t / 60), m: t % 60 };
}

/** Inline hours + minutes picker */
function DurPicker({ total, onChange, inputStyle }) {
  const { h, m } = splitDur(total);
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <input
          ...
          onChange={e => onChange(Math.max(0, Number(e.target.value)) * 60 + m)}
        />
        <span style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 600 }}>h</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <select
        ...
        >
          {MIN_OPTS.map(v => <option key={v} value={v}>{String(v).padStart(2, '0')}</option>)}
        </select>
        <span style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 600 }}>min</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={m.overlay}>
      <div style={m.box} className="scale-in">
        ...
        <div style={m.label}>
          Custom Duration
          <DurPicker total={durTotal} onChange={setDurTotal} inputStyle={m.input} />
        </div>
...
export default function OwnerServices() {
  ...
  const orderedAvailableCats = [
    ...CAT_ORDER.filter(c => availableByCategory[c]),
    ...Object.keys(availableByCategory).filter(c => !CAT_ORDER.includes(c)),
  ];
  ...
  const attach = async () => {
    ...
    try {
      ...
      const maxOrder = attached.reduce((m, s) => Math.max(m, s.display_order ?? 0), 0);
      await api.post(`/salons/${salon.id}/services/`, { service: Number(toAdd), display_order: maxOrder + 1 });
  ...
  const createCustom = async () => {
    if (!form.name || !form.price || !form.durTotal) return setError('All fields are required.');
    if (Number(form.durTotal) < 1) return setError('Duration must be at least 1 minute.');
  ...
  const handleDragStart = (e, ssId) => {
    dragId.current = ssId;
    e.dataTransfer.effectAllowed = 'move';
    // ghost image: native browser ghost is fine
  };

  const handleDragOver = (e, ssId) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';
    if (ssId !== dragOverId) setDragOverId(ssId);
  };
  ...
  const handleDrop = async (e, targetId) => {
    ...
    setAttached(reordered); // optimistic update
    try {
      await api.post(`/salons/${salon.id}/services/reorder/`, {
  ...
  return (
    <div>
      {editTarget && (
        <EditModal ss={editTarget} onSave={saveEdit} onClose={() => setEditTarget(null)} />
      )}
      ...
      {showCreate && (
        <div style={s.createCard} className="scale-in">
          ...
          <div style={s.createGrid}>
            ...
            <div style={s.fLabel}>
              Duration
              <DurPicker total={form.durTotal} onChange={v => setForm(f => ({ ...f, durTotal: v }))} inputStyle={s.fInput} />
            </div>
          </div>
      ...
      {attached.length > 0 && (
        <p style={s.dragHint}>⠿ Drag cards to reorder · changes save instantly</p>
      )}
          ...
          <div key={cat} style={s.catSection}>
            ...
            <div style={s.grid}>
              {items.map(ss => {
                ...
                const isDragged = dragId.current === ss.id;
                const isOver    = dragOverId === ss.id;
                return (
                  <div
                    key={ss.id}
                    draggable
                  ...
                  >
                    ...
                    <div style={s.cardBody}>
                      ...
                      <div style={s.meta}>
                        <span style={s.metaItem}>⏱ {formatDuration(ss.effective_duration)}</span>
                      ...
                      {ss.description ? <div style={s.descText}>{ss.description}</div> : null}
...
const s = {
  ...
  addBtn: { padding: '10px 22px', background: 'linear-gradient(135deg, #0D9488 0%, #14B8A8 50%, #0D9488 100%)', color: '#fff', border: 'none', borderRadius: 11, cursor: 'pointer', fontWeight: 700, fontSize: 14, boxShadow: '0 4px 14px rgba(13,148,136,.3)', fontFamily: "'DM Sans', sans-serif" },
  createBtn: { padding: '10px 22px', background: 'var(--surface)', color: '#0D9488', border: '1.5px solid rgba(13,148,136,.3)', borderRadius: 11, cursor: 'pointer', fontWeight: 700, fontSize: 14, fontFamily: "'DM Sans', sans-serif" },
  createCard: { background: 'var(--surface)', borderRadius: 18, padding: '22px 24px', border: '1px solid var(--border)', boxShadow: '0 4px 16px rgba(13,148,136,.08)', marginBottom: 24 },
  ...
  dragHint: { fontSize: 12, color: 'var(--text-muted)', marginBottom: 20, marginTop: -8 },
  ...
  card: { background: 'var(--surface)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(13,148,136,.06)', border: '1px solid var(--border)', transition: 'opacity .15s, outline .1s' },
  ...
  rmTitle: { fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 22, fontWeight: 700, color: 'var(--text)', margin: '0 0 10px' },
  rmSub: { fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.65, margin: '0 0 16px' },
  rmWarnBlock: { background: '#FFFBEB', border: '1px solid #FCD34D', borderRadius: 12, padding: '12px 16px', textAlign: 'left', marginBottom: 20 },
  rmWarnTitle: { fontSize: 13, fontWeight: 700, color: '#92400E', marginBottom: 8 },
  rmStaffList: { display: 'flex', flexWrap: 'wrap', gap: 6 },
  rmStaffChip: { fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: 'rgba(217,119,6,.1)', color: '#92400E', border: '1px solid #FCD34D' },
  rmActions: { display: 'flex', gap: 10, justifyContent: 'center', marginTop: 6 },
  rmCancelBtn: { padding: '10px 22px', background: 'var(--surface2)', border: '1.5px solid var(--border)', borderRadius: 12, cursor: 'pointer', fontSize: 14, fontWeight: 500, color: 'var(--text-muted)', fontFamily: "'DM Sans', sans-serif" },
  rmDeleteBtn: { padding: '10px 24px', background: '#DC2626', border: 'none', borderRadius: 12, cursor: 'pointer', fontSize: 14, fontWeight: 700, color: '#fff', boxShadow: '0 4px 14px rgba(220,38,38,.35)', fontFamily: "'DM Sans', sans-serif" },
};
...
const m = {
  ...
  saveBtn:   { padding: '9px 20px', background: '#0D9488', color: '#fff', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 13, fontFamily: "'DM Sans', sans-serif", boxShadow: '0 4px 12px rgba(13,148,136,.3)' },
